...
export default function Sidebar() {
  ...
  return (
    <>
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 flex h-screen w-72 flex-col border-r border-yellow-600/20 bg-gradient-to-b from-[#1c1c1c] via-[#181818] to-[#141414] shadow-[4px_0_24px_rgba(0,0,0,0.4)] transition-transform duration-300 ease-in-out md:static md:translate-x-0 ${isOpen ? "translate-x-0" : "-translate-x-full"}`}>
        <div className="border-b border-yellow-600/20 p-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold tracking-wide text-[#D4AF37]">Blink & Bliss</h1>
              <p className="text-xs uppercase tracking-widest text-gray-500">CRM System</p>
            </div>
          </div>
        </div>
        ...
        <div className="border-t border-yellow-600/20 p-4">
          <button onClick={logout} className="flex w-full items-center gap-3 rounded-xl px-4 py-3 text-sm font-medium text-red-400 transition-all duration-200 hover:bg-red-500/10 hover:pl-5">
