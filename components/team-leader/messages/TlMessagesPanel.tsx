...
import {
  Send, AtSign, X, ExternalLink, Paperclip, ArrowLeft,
  MessageSquare, ShieldAlert, ChevronUp, Trash2,
} from "lucide-react";
import type { ChatMessage, MentionLead } from "@/types/lead";
import { useUnreadCounts } from "@/hooks/useUnreadCounts";
import { subscribeToMessages, subscribeToTyping, subscribeToSettingsChanges } from "@/lib/realtime";
import ChatImage, { isImageFile } from "@/components/chat/ChatImage";
import ImagePreview from "@/components/chat/ImagePreview";

type Admin = { id: string; name: string; phone: string | null; role?: string };

export default function TlMessagesPanel({ currentUserId }: { currentUserId: string }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const r = await fetch("/api/team-leader/messages/contacts", { cache: "no-store" });
  ...
  useEffect(() => {
    fetch("/api/team-leader/settings")
      .then((r) => r.json())
      .then((j) => {
        if (j.data?.tlMessageEnabled === false) setDisabled(true);
      })
      .catch(() => {});
  }, []);
  ...
  useEffect(() => {
    ...
    const { unsubscribe, sendTyping } = subscribeToTyping(
      `tl:${currentUserId}`,
      (payload) => {
        if (payload.isTyping && Date.now() - payload.ts < 5000) setIsTyping(true);
  ...
  useEffect(() => {
    ...
    fetch("/api/team-leader/messages", {
      method: "PATCH",
  ...
  const fetchMessages = useCallback(async (adminId: string) => {
    ...
    try {
      const r = await fetch(`/api/team-leader/messages?userId=${adminId}`, { cache: "no-store" });
      const j = await r.json();
      if (j.success && j.data) {
        shouldAutoScrollRef.current = true;
  ...
  const loadOlderMessages = useCallback(async () => {
    ...
    try {
      const r = await fetch(
        `/api/team-leader/messages?userId=${selectedAdmin.id}&cursor=${encodeURIComponent(oldest.id)}`,
        { cache: "no-store" },
      );
      const j = await r.json();
      if (j.success && j.data) {
        ...
        setMessages((prev) => {
          ...
          return [...j.data.filter((m: any) => !existingIds.has(m.id)), ...prev];
        });
        setHasMore(Boolean(j.hasMore));
  ...
  function onMessagesScroll(e: React.UIEvent<HTMLDivElement>) {
    if (e.currentTarget.scrollTop < 120) loadOlderMessages();
  }
  ...
  async function clearAllMessages() {
    ...
    try {
      ...
      const j = await r.json();
      if (j.success) { setMessages([]); toast.success("Messages cleared."); }
  ...
  function onInputChange(val: string) {
    ...
    if (typingRef.current) {
      ...
      typingTimerRef.current = setTimeout(() => typingRef.current?.sendTyping(false, ""), 3000);
    }
    ...
    if (atIdx !== -1 && !val.slice(atIdx + 1).includes(" ")) {
      ...
      mentionTimerRef.current = setTimeout(() => {
        ...
        if (q.length >= 1)
          fetch(`/api/team-leader/messages?query=${encodeURIComponent(q)}`, { cache: "no-store" })
            .then((r) => r.json())
            .then((j) => { if (j.leads) setMentionResults(j.leads.slice(0, 5)); })
            .catch(() => {});
      }, 100);
      return;
    }
  ...
  function selectMentionLead(lead: MentionLead) {
    setNewMsg((prev) => prev.slice(0, prev.lastIndexOf("@")) + `@${lead.name || lead.phone} `);
  ...
  async function sendMessage() {
    ...
    const tempMsg: ChatMessage = {
      id: tempId, content, senderId: currentUserId, receiverId: selectedAdmin.id,
      leadId, fileUrl: null, fileName: null, fileSize: null, isRead: false, readAt: null,
  ...
  async function doUploadAndSend(file: File, caption: string) {
    ...
    const tempMsg: ChatMessage & { _sending?: boolean } = {
      ...
      receiverId: selectedAdmin.id, leadId: mentionedLead?.id || null,
      fileUrl: blobUrl, fileName: file.name, fileSize: file.size,
      isRead: false, readAt: null, createdAt: new Date().toISOString(), _sending: true,
    };
    shouldAutoScrollRef.current = true;
    ...
    try {
      ...
      if (uploadJson.success) {
        ...
        const m = await fetch("/api/team-leader/messages", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            receiverId: selectedAdmin.id, content, leadId: mentionedLead?.id || null,
            fileUrl: uploadJson.data.fileUrl, fileName: uploadJson.data.fileName,
  ...
  return (
    <div className="flex h-full rounded-xl sm:rounded-[28px] border border-[#D4AF37]/20 bg-gradient-to-br from-[#171717] to-[#0d0d0d] shadow-lg overflow-hidden">
      ...
      <div className={`${showChat ? "hidden" : "flex"} sm:flex w-full sm:w-64 flex-shrink-0 flex-col border-r border-white/10 bg-black/20`}>
        <div className="p-3 sm:p-4 border-b border-white/10 flex items-center justify-between">
          <h2 className="text-sm font-semibold text-[#D4AF37] flex items-center gap-2">
            <MessageSquare size={16} /> Admin Chat
          </h2>
          {unreadMessages > 0 && (
            <span className="rounded-full bg-red-500 px-2 py-0.5 text-[10px] font-bold text-white">{unreadMessages}</span>
          )}
        </div>
        <div className="flex-1 overflow-y-auto">
          {admins.length === 0 ? (
            <div className="p-4 text-center text-white/40 text-xs">No admins available.</div>
          ) : (
            admins.map((admin) => (
              <button
                key={admin.id}
              ...
              >
                ...
                <div className="flex items-center gap-1.5 min-w-0">
                  <span className="text-sm text-white font-medium truncate">{admin.name}</span>
                  <span className="text-[9px] text-red-400 bg-red-400/10 rounded-full px-1.5 py-px font-bold border border-red-400/20 shrink-0">Admin</span>
                </div>
              </button>
            ))
          )}
      ...
      <div className={`${!showChat ? "hidden" : "flex"} sm:flex flex-1 flex-col min-w-0`}>
        {selectedAdmin ? (
          <>
            <div className="p-3 sm:p-4 border-b border-white/10 flex items-center gap-2 sm:gap-3 shrink-0">
              <button onClick={() => setShowChat(false)} className="sm:hidden p-1.5 rounded-lg text-white/60 hover:text-white">
                <ArrowLeft size={18} />
              </button>
              <div className="h-8 w-8 sm:h-9 sm:w-9 rounded-full bg-[#D4AF37]/20 flex items-center justify-center text-[#D4AF37] text-xs font-bold shrink-0">
              ...
              <div className="flex items-center gap-1.5 min-w-0">
                <span className="font-medium text-white truncate text-sm sm:text-base">{selectedAdmin.name}</span>
                <span className="text-[9px] text-red-400 bg-red-400/10 rounded-full px-1.5 py-px font-bold border border-red-400/20 shrink-0">Admin</span>
              </div>
            ...
            <div ref={scrollContainerRef} onScroll={onMessagesScroll} onPaste={handlePaste}
              ...
              {loading ? (
                ...
                <div className="flex items-start justify-center pt-8 h-full text-white/40 text-center">
                  <div>
                    ...
                    <p className="text-[10px] mt-1">Type @ to mention a lead!</p>
                  </div>
                </div>
              ) : (
                <>
                  ...
                  {messages.map((msg) => {
                    ...
                    return (
                      <div key={msg.id} className={`flex ${isMine ? "justify-end" : "justify-start"}`}>
                        <div className={`max-w-[85%] sm:max-w-[75%] rounded-2xl px-3 py-2 sm:px-4 sm:py-2.5 text-xs sm:text-sm overflow-hidden ${
                          isMine
                          ...
                          {msg.fileUrl && isImageFile(msg.fileName) ? (
                            ...
                            <a href={msg.fileUrl} download={msg.fileName} target="_blank" rel="noopener"
                          ...
                          {msg.lead && (
                            <a href={`/team-leader/leads?leadId=${msg.lead.id}`}
                              ...
                              <span className="font-medium truncate max-w-[100px]">{msg.lead.name || msg.lead.phone}</span>
                            </a>
                          )}
            ...
            {isTyping && (
              <div className="px-3 sm:px-4 py-1 shrink-0">
                <p className="text-[10px] text-emerald-400/70 italic animate-pulse">Admin is typing...</p>
              </div>
            )}
            ...
            {mentionedLead && (
              <div className="mx-3 sm:mx-4 mb-1 flex items-center gap-1.5 rounded-xl bg-blue-500/10 border border-blue-500/25 px-2.5 py-1 shrink-0">
                ...
                <span className="text-[11px] text-blue-300 font-medium truncate">{mentionedLead.name || mentionedLead.phone}</span>
                <button onClick={() => setMentionedLead(null)} className="ml-auto text-blue-400/60"><X size={12} /></button>
              </div>
            )}
            ...
            {showMentionSearch && mentionResults.length > 0 && (
              <div className="mx-3 sm:mx-4 mb-1 rounded-xl border border-white/10 bg-[#1a1a1a] max-h-32 overflow-y-auto shrink-0">
                {mentionResults.map((lead) => (
                  <button key={lead.id} onClick={() => selectMentionLead(lead)}
                    ...
                    <span className="text-white font-medium truncate">{lead.name || "No Name"}</span>
                    <span className="text-[10px] text-white/30 ml-auto shrink-0">{lead.phone}</span>
                  </button>
                ))}
              </div>
            )}
            ...
            <div className="p-2 sm:p-4 border-t border-white/10 shrink-0 bg-gradient-to-t from-black/30 to-transparent">
              <form onSubmit={(e) => { e.preventDefault(); sendMessage(); }} className="flex gap-1.5 sm:gap-2 items-center">
                <div className="flex-1 relative min-w-0">
                  <input ref={inputRef} value={newMsg} onChange={(e) => onInputChange(e.target.value)}
                  ...
                  <button type="button" onClick={() => { setNewMsg((p) => p + "@"); setShowMentionSearch(true); }}
