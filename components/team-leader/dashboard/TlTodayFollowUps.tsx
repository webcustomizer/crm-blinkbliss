...
export default function TlTodayFollowUps({ followUps }: { followUps: FollowUp[] }) {
  return (
    <div className="w-full rounded-2xl border border-[#D4AF37]/20 bg-[#161616] p-4">
      <div className="flex items-center justify-between">
        <h2 className="text-base font-semibold text-white sm:text-lg">Today&apos;s Follow Ups</h2>
      ...
      {followUps.length === 0 ? (
        ...
        <div className="mt-4 max-h-[420px] space-y-3 overflow-y-auto overflow-x-hidden">
          {followUps.map((lead) => (
            <div key={lead.id}
              className="group rounded-xl border border-[#D4AF37]/10 bg-gradient-to-b from-[#141414] to-[#0d0d0d] p-3.5 transition-colors duration-300 hover:border-[#D4AF37]/25"
            >
              <div className="flex items-start justify-between gap-2">
                <div className="min-w-0">
                  <p className="truncate text-sm font-semibold tracking-wide text-white">{lead.name || "Unknown Lead"}</p>
                  <p className="mt-1.5 flex items-center gap-1.5 text-xs text-zinc-500">
                    <Phone size={12} className="text-zinc-600" />
                    {lead.phone}
                  </p>
                </div>
                <span className="shrink-0 rounded-full border border-[#D4AF37]/20 bg-[#D4AF37]/[0.08] px-2.5 py-1 text-[10px] font-medium uppercase tracking-wider text-[#D4AF37]">
                  {lead.status}
                </span>
              </div>
              ...
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-1.5 text-xs text-zinc-500">
                  ...
                  <span>{lead.nextFollowUp ? formatDateTime(lead.nextFollowUp) : "No follow up"}</span>
                </div>
                <button type="button" onClick={() => smartCall(lead.phone)}
                  title="Call (via WhatsApp if available)"
                  className="inline-flex shrink-0 items-center gap-2 rounded-full border border-[#D4AF37]/25 bg-gradient-to-b from-[#1e1e1e] to-[#151515] py-1.5 pl-2 pr-3.5 text-xs font-medium tracking-wide text-[#e8e8e8] shadow-sm transition-all duration-300 hover:border-[#D4AF37]/50 hover:shadow-[0_0_14px_rgba(212,175,55,0.18)] active:scale-95"
                >
              ...
              {lead.remarks && (
                <p className="mt-3 line-clamp-2 text-xs leading-relaxed text-zinc-500">{lead.remarks}</p>
              )}
            </div>
          ))}
        </div>
