...
export default function ChangePasswordCard({ forceChange = false }: { forceChange?: boolean }) {
  ...
  useEffect(() => {
    fetch("/api/team-leader/settings", { cache: "no-store" })
      .then((r) => r.json())
      .then((j) => {
        if (j.data) {
  ...
  async function handleChangePassword() {
    ...
    try {
      setLoading(true);
      const res = await fetch("/api/team-leader/profile/password", {
        method: "POST",
      ...
      if (forceChange) {
        ...
        setTimeout(() => { window.location.href = "/team-leader/dashboard"; }, 1500);
      }
    } catch (error: any) {
